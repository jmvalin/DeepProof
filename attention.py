...
class Attention(Layer):
    ...
    def build(self, input_shape):
        ...
        self.query_kernel = self.add_weight(shape=(query_dim, self.units),
                                      initializer=self.kernel_initializer,
                                      name='query_kernel',
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
        self.key_kernel = self.add_weight(shape=(key_dim, self.units),
                                      initializer=self.kernel_initializer,
                                      name='key_kernel',
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
        if self.use_bias:
            self.query_bias = self.add_weight(shape=(self.units,),
                                        initializer=self.bias_initializer,
                                        name='query_bias',
                                        regularizer=self.bias_regularizer,
                                        constraint=self.bias_constraint)
            self.key_bias = self.add_weight(shape=(self.units,),
                                        initializer=self.bias_initializer,
                                        name='key_bias',
                                        regularizer=self.bias_regularizer,
                                        constraint=self.bias_constraint)
    ...
    def call(self, inputs):
        ...
        q = K.dot(queries, self.query_kernel)
        k = K.dot(keys, self.key_kernel)
        if self.use_bias:
            q = K.bias_add(q, self.query_bias)
