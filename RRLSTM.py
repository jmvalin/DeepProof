...
from keras.layers import Input, LSTM, Dense, Embedding, Reshape, Layer, RNN, LSTMCell, GRUCell, InputSpec
...
class RRLSTMCell(Layer):
    """Cell class for the RRLSTM layer.
    ...
    def build(self, input_shape):
        ...
        self.kernel2 = self.add_weight(shape=(self.rank, self.units * 4),
                                      name='kernel2',
                                      initializer=self.kernel_initializer,
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
        self.recurrent_kernel2 = self.add_weight(
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            if 0 < self.dropout < 1.:
                ...
                inputs_c = inputs
                inputs_o = inputs
            x_i = K.dot(K.dot(inputs_i, self.kernel_i1), self.kernel_i2)
            x_f = K.dot(K.dot(inputs_f, self.kernel_f1), self.kernel_f2)
            x_c = K.dot(K.dot(inputs_c, self.kernel_c1), self.kernel_c2)
            x_o = K.dot(K.dot(inputs_o, self.kernel_o1), self.kernel_o2)
            if self.use_bias:
            ...
            if 0 < self.recurrent_dropout < 1.:
                ...
                h_tm1_c = h_tm1
                h_tm1_o = h_tm1
            i = self.recurrent_activation(x_i + K.dot(K.dot(h_tm1_i,
                                                      self.recurrent_kernel_i1), self.recurrent_kernel_i2))
            f = self.recurrent_activation(x_f + K.dot(K.dot(h_tm1_f,
                                                      self.recurrent_kernel_f1), self.recurrent_kernel_f2))
            c = f * c_tm1 + i * self.activation(x_c + K.dot(K.dot(h_tm1_c,
                                                            self.recurrent_kernel_c1), self.recurrent_kernel_c2))
            o = self.recurrent_activation(x_o + K.dot(K.dot(h_tm1_o,
                                                      self.recurrent_kernel_o1), self.recurrent_kernel_o2))
        else:
            if 0. < self.dropout < 1.:
                inputs *= dp_mask[0]
    ...
    def get_config(self):
        ...
        base_config = super(RRLSTMCell, self).get_config()
...
class RRLSTM(RNN):
    ...
    # References
        ...
        - [Learning to forget: Continual prediction with RRLSTM](http://www.mitpressjournals.org/doi/pdf/10.1162/089976600300015015)
    ...
    def __init__(self, units,
        ...
        super(RRLSTM, self).__init__(cell,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        ...
        return super(RRLSTM, self).call(inputs,
                                      mask=mask,
                                      training=training,
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        base_config = super(RRLSTM, self).get_config()
